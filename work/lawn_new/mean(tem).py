# -*- coding: utf-8 -*-
"""
Created on Mon Nov 28 17:30:28 2022
...
for file in glob.glob(path+os.sep+'*.csv'):
    ...
    plt.plot(tem[1],tem['mean'],color=colors[j],label=titles[j])
    # plt.plot(NDVI[1],NDVI['min'],'-.',color=colors[j])
    # plt.plot(NDVI[1],NDVI['max'],'--',color=colors[j])
    plt.fill_between(tem[1], tem['max'], y2=tem['min'], color=colors[j], alpha = 0.2)
