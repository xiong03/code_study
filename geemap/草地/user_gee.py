...
def clip_dow_merge(geo: ee.Geometry, image: ee.Image, outfile: str, scale: int,
                   data_type_bytes: int, crs='EPSG:4326', max_bytes=40000000, flag=True):
    """
    ...
    if polys:
        ...
        while files != count:
            ...
            files = len(glob(path+"/*.tif"))
        if flag:
            merge_img(path, outfile, x_offset, y_offset, height, width, transform, scale, crs)
    else:
...
def merge_img(path: str, outfile, x_offset, y_offset, height, width, transform, scale, crs):
    """
    ...
    if out_meta['crs'] is None:
        out_meta.update({'crs':CRS.from_string(crs)})
